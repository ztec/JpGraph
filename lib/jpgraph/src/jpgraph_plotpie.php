...
class PiePlot {
 ...
 "sand"   => array(27,168,34,170,19,50,65,72,131,209,46,393));
    ...
    function __construct($data) {
        ...
        $this->title = new Text("");
        $this->title->SetFont(FF_TIMES,FS_BOLD);
...
//        return;
    ...
    function SetSizeSlice($slices , $l) {
        ...
        if( isset($slices) ){
           ...
           // $radius = 0;
            ...
            foreach ($slices as $slice){
                $this->labels[] = $slice;
                $this->radius_slices[]  = round(($slice *$radius) /100 );
            }
    ...
    //    $img->CakeSlice(225,125,$radius,$radius,360,360 , '#ddd');
...
//            echo "Slice#$i: ang1=$_sa , ang2=$_ea, la=$_la, color=$slicecolor<br>";
            ...
            if( $aaoption !== 2 ) {
                ...
                $slice_radius = $this->radius_slices[$i];
                if( abs($_ea-$_sa) >= 1 || $d == $sum ) {

                    $img->CakeSlice($xcm,$ycm,$slice_radius,$slice_radius,$_sa,$_ea,$slicecolor,$arccolor);
                }
            }
